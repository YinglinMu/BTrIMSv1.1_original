...
PROGRAM back_traj
	...
	do dd = 1, totdays
		...
		call calc_pw(mix(:,:,:,datatotsteps-datadaysteps:),pres(:,:,:,datatotsteps-datadaysteps:),psfc(:,:,datatotsteps-datadaysteps:),ptop,pw)
...
!$OMP PARALLEL DEFAULT(PRIVATE) SHARED(pw,tpw,u,v,w,pres,temp,psfc,evap,precip,mix,mixtot,pbl_lev,lat2d,lon2d,orec,outncid,wvcid,wvc2id,xlocid,ylocid,dayid,opreid,wsmask,daytsteps,totsteps,indatatsteps,datadaysteps,datatotsteps,dim_i,dim_j,dim_k,sday,smon,syear,mon,year,day,dd,totpts,ssdim)
					...
					do mm = 1, par_release(tt)
						...
						do nn = totsteps-daytsteps+tt, 2, -1
							...
							if (SUM(WV_cont)>=1.) then
								!print *,"all precip accounted (torec,wv_cont) ",torec,SUM(WV_cont)
