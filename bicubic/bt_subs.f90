...
!***********************************************************************
	...
	SUBROUTINE new_out_file(outncid,wvcid,xlocid,ylocid,dayid,opreid,daynum,lat2d,lon2d)
	!----------------------------------------------
	...
	!--------------------------------------------------------
		...
		INTEGER, INTENT(INOUT) :: outncid,wvcid,xlocid,ylocid,dayid,opreid
	...
	REAL FUNCTION cubic_interp(x, x_in, f_in)
    IMPLICIT NONE

    REAL, INTENT(IN) :: x_in(4), f_in(4)
    REAL, INTENT(IN) :: x
    REAL :: diff(4, 4)
    INTEGER :: i, j
    ...
    DO j = 2, 4
        DO i = 1, 4 - j + 1
            diff(i, j) = (diff(i + 1, j - 1) - diff(i, j - 1)) / (x_in(i + j - 1) - x_in(i))
        END DO
    END DO

    ! calculate the cubic interpolation
    cubic_interp = diff(1, 1)
    DO i = 2, 4
        cubic_interp = cubic_interp + diff(1, i) * PRODUCT(x - x_in(1:i-1))
    END DO

	END FUNCTION cubic_interp

	SUBROUTINE bicubic_interp(var2d,lon2d,lat2d,x,y,par_lon,par_lat,var)   
		!------------------------------------------------------------------
		...
		!------------------------------------------------------------------
			...
			REAL :: fac,t,u
			REAL, DIMENSION(:) :: column_vals(4)
			INTEGER :: xx,yy,xbdy,ybdy,x,y,i
			...
			if ((xbdy/=0.AND.ybdy/=0)) then
				...
				var = cubic_interp(par_lat, lat2d(xx,yy-1:yy+2),var2d(xx,yy-1:yy+2))
			else if (ybdy/=0) then
				var = cubic_interp(par_lon, lon2d(xx-1:xx+2,yy),var2d(xx-1:xx+2,yy))
			else
				do i = 1, 4
					column_vals(i) = cubic_interp(par_lat, lat2d(xx,yy-1:yy+2), var2d(xx,yy-1:yy+2))
				end do
			
				! interpolation in y direction, for every row 
				var = cubic_interp(par_lon, lon2d(xx-1:xx+2,yy),column_vals)
			end if
	...
	!--------------------------------------------------------------------------
		...
		call bicubic_interp(u(:,:,par_lev,2),lon2d,lat2d,xx,yy,lon,lat,u_back) ! where lon2d/lat2d are the subgrids
		call bicubic_interp(v(:,:,par_lev,2),lon2d,lat2d,xx,yy,lon,lat,v_back)
		...
		pr = par_pres
		call bicubic_interp(temp(:,:,par_lev),lon2d,lat2d,xx,yy,lon,lat,temp_par)
		call bicubic_interp(w(:,:,par_lev,2),lon2d,lat2d,xx,yy,lon,lat,w_par)
		...
		call bicubic_interp(u(:,:,lev,1),lon2d,lat2d,xx,yy,lon,lat,u_for)
		call bicubic_interp(v(:,:,lev,1),lon2d,lat2d,xx,yy,lon,lat,v_for)
		...
		call bicubic_interp(temp(:,:,par_lev),lon2d,lat2d,xx,yy,par_lon,par_lat,temp_par)
		call bicubic_interp(w(:,:,par_lev,1),lon2d,lat2d,xx,yy,par_lon,par_lat,w_par)
