...
!***********************************************************************
	...
	SUBROUTINE calc_par_pot_temp(par_temp,par_pres,par_pot_temp)
		...
		! calculate the potential temperature
		!
		par_pot_temp = par_temp * ((P0/par_pres)**(Rd/Cp)) 
	...
	SUBROUTINE calc_par_eq_pot_temp(par_mix,par_mixtot,par_temp,par_pres,par_eq_pot_temp)
	!------------------------------------------------
	! SUBROUTINE UNUSED
	...
	!-----------------------------------------------------
		...
		REAL, INTENT(IN) :: par_mix,par_mixtot,par_temp,par_pres

		REAL, INTENT(OUT) :: par_eq_pot_temp


		REAL :: par_es,par_mix_s

		!
		! use Teten's formula to calculate the saturation vapor pressure (in stull)
		!
		par_es = 611 * exp((17.2694*(par_temp - 273.16))/(par_temp- 35.86))

		!
		! calculate the saturated mixing ratio everywhere (everywhere!)
		!
		par_mix_s = (Rd/Rv)*par_es/(par_pres - par_es)

		!
		! calculate the equivalent potential temperature (in atmospheric convection, Emanuel)
		!
		par_eq_pot_temp = par_temp * ((P0/par_pres)**(Rd/(Cp+Cl*par_mixtot))) * &
				par_mix_s**(-1.*par_mix*Rv/(Cp+Cl*par_mixtot)) * &
				exp(Lv*par_mix/((Cp+Cl*par_mixtot)*par_temp))
	...
	END SUBROUTINE calc_par_eq_pot_temp
