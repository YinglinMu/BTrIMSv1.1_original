...
! Air parcels move kinetically in horizontal, but along the equivalent potential temperature iso-surface in vertical.
! The PBL is NOT split from the upper atmosphere, i.e. the whole column is assumed to be well-mixed at the time step scale, 'ET-mixing' identification is used along the trajectory.
...
PROGRAM back_traj
	...
	REAL :: par_mix,par_temp,par_eq_pot_temp  !new added
	INTEGER :: datatstep
	...
	ALLOCATE( precip(dim_j,dim_i,datadaysteps), &
		...
		eq_pot_temp(dim_j,dim_i,dim_k,datatotsteps), &
	...
	do dd = 1, totdays
        ...
        !call calc_tpw_pbl(mixtot,pres,psfc,tpw,pbl_lev) ! this is the total precipitable water within the boundary layer

		call calc_eq_pot_temp(mix,mixtot,temp,pres,eq_pot_temp)
...
!$OMP PARALLEL DEFAULT(PRIVATE) SHARED(pw,tpw,u,v,w,pres,temp,psfc,evap,precip,mix,mixtot,pbl_lev,lat2d,lon2d,orec,outncid,wvcid,xlocid,ylocid,dayid,opreid,wsmask,daytsteps,totsteps,indatatsteps,datadaysteps,datatotsteps,dim_i,dim_j,dim_k,sday,smon,syear,mon,year,day,dd,totpts,ssdim)
		...
		ALLOCATE( WV_cont(dim_j,dim_i),WV_cont_day(dim_j,dim_i), &
				...
				eq_pot_temp_then(ssdim,ssdim,dim_k), &
			...
			if (SUM(precip(xx,yy,:))>minpre) then
				...
				do tt = 1, daytsteps
					...
					do mm = 1, par_release(tt)
						...
						! Calculate the parcel vapor mixing ratio. This is used in the calculation of equivalent potential temperature.
						par_mix = lin_interp(mix(xx,yy,par_lev,ttdata:ttdata+1),ttfac)
						...
						! Calculate parcel temp.This is used in the calculation of new parcel level in new_parcel_level
						par_temp = lin_interp(temp(xx,yy,par_lev,ttdata:ttdata+1),ttfac)

						! Calculate parcel potential temperature,This is used in the calculation of new parcel level in new_parcel_level_pt.
						call calc_par_eq_pot_temp(par_mix,par_q,par_temp,par_pres,par_eq_pot_temp)
						...
						do nn = totsteps-daytsteps+tt, 2, -1
							...
							eq_pot_temp_then(:,:,:) = lin_interp3D(eq_pot_temp(ssx:ssx+ssdim,ssy:ssy+ssdim,:,nnMM5:nnMM5+1),nnfac)
							...
							call implicit_back_traj(unow,vnow,wnow,tempnow,eq_pot_temp_then,pres_then,psfc_then,lon2d(ssx:ssx+ssdim-1,ssy:ssy+ssdim-1),lat2d(ssx:ssx+ssdim-1,ssy:ssy+ssdim-1),par_lon,par_lat,par_lev,par_eq_pot_temp,par_pres,par_q,threadnum)
