...
!***********************************************************************
	...
	SUBROUTINE new_parcel_level_w(par_pres,pres,w,mix,lev,psfc)
	!-------------------------------------------------------------------------
	...
	SUBROUTINE implicit_back_traj_w(u,v,w,pres,psfc,lon2d,lat2d, &
					par_lon,par_lat,par_lev, &
	...
	!--------------------------------------------------------------------------
		...
		REAL :: lon,lat,u_back,v_back,w_par,u_for,v_for,pr
		...
		if (lev==0) then
		  print *,'parcel lev=0',par_lev,lev,par_pres,thread
		  STOP
		end if
